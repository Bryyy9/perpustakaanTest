  ...
  return (
    <section className="space-y-6">
      <div className="rounded-3xl border border-slate-200 bg-white p-6 shadow-sm">
        ...
        <h2 className="text-2xl font-semibold text-slate-900">{book?.judul_buku || 'Memuat...'}</h2>
      </div>
      ...
      <div className="grid gap-6 lg:grid-cols-[320px_minmax(0,1fr)]">
        <div className="overflow-hidden rounded-3xl border border-slate-200 bg-white shadow-sm">
          {book?.gambar_buku ? (
            ...
            <div className="flex h-96 items-center justify-center bg-slate-100 text-slate-400">
              Tidak ada gambar
            </div>
          )}
        </div>
      ...
      {query.isLoading && <p className="text-sm text-slate-500">Memuat detail buku...</p>}
    </section>
  )
}
